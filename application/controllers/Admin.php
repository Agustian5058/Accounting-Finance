...
class Admin extends CI_Controller {
		...
		if ($argv == 'edit'){
			if ($argv1 == ""){
				...
				$dataPenyewaan = array(
					...
					'pph'             => $data_penyewaan->pph_penyewaan,
	...
	public function pengeluaran($argv = '', $argv1 = ''){
		if ($argv == ''){
			$query = 'SELECT count(pengeluaran_id) as total FROM pengeluaran';
			$hasil = $this->db->query($query)->row();

			$data = 'Halaman - 500 Data &nbsp;<select class="col-md-5 form-control" id="pageCount" >';
			$i = 1;
			while ( $hasil->total > 0) {
				if ($hasil->total >= 500) {
					$x = $hasil->total - 500;
					$data .= '<option value="'.$i.'">'.$i.' - '.($i+499).'</option>';
					$hasil->total = $hasil->total - 500;
					$i = $i + 500;
				} else {
					$data .= '<option value="'.$i.'">'.$i.' - '.($i+$hasil->total-1).'</option>';
					$hasil->total = $hasil->total - $hasil->total;
					$i = $i + $hasil->total;
				}
			}
			$data .= '</select>';
	...
	public function lap_fee($argv = '', $argv1 = ''){
		...
		if ($argv == 'generate'){
			...
			if ($jenis_biaya == '') {
				...
				$pengeluaran_fee              = $this->Md_pengeluaran->getPengeluaranFeeByRangeTglPemasukanAndJenisBiaya($tgl_awal,$tgl_akhir,$jenis_biaya);
			}
			...
			$pageData['periode'] 				 = date('d-M-Y',strtotime($tgl_awal)).' - '.date('d-M-Y',strtotime($tgl_akhir));
			$pageData['jenis_biaya'] 			 = $this->Md_sasaran_dana->getSasaranDanaById($jenis_biaya);
			$pageData['view']                    = 'paper';
	...
	public function lap_penyewaanalat($argv = '', $argv1 = ''){
		if ($argv == ''){
			$pageData['view']     = 'form';
		}
		...
		if ($argv == 'generate'){
			...
			//dont allow if coming without parameter post value
			if(!$this->input->post('tgl_awal') || !$this->input->post('tgl_akhir')) redirect(base_url().'index.php/admin/lap_penyewaanalat','refresh');

			if (date('Y-m-d',strtotime($tgl_awal)) > date('Y-m-d',strtotime($tgl_akhir))){
				$this->session->set_flashdata('statuserror', 'Periode tanggal yang anda pilih tidak benar');
				redirect(base_url().'index.php/admin/lap_penyewaanalat','refresh');
			}
			$data            = array();
			$pemasukan       = $this->Md_pemasukan->getPemasukanByTgl($tgl_awal, $tgl_akhir);
		...
		//select option view
		if ($argv1 == 'excel'){
			if (!$pageData['excel']) redirect(base_url().'index.php/admin/lap_penyewaanalat','refresh');
			$this->load->view('page/excel_lap_penyewaanalat', $pageData);
		}else{
			$pageData['page_name'] = 'lap_penyewaanalat'; 
			$this->load->view('index', $pageData);
		}		
	}

	public function lap_sparepart($argv = '', $argv1 = ''){
		if ($argv == ''){
			$pageData['view']     = 'form';
		}
		...
		if ($argv == 'generate'){
			...
			//dont allow if coming without parameter post value
			if(!$this->input->post('tgl_awal') || !$this->input->post('tgl_akhir')) redirect(base_url().'index.php/admin/lap_sparepart','refresh');

			if (date('Y-m-d',strtotime($tgl_awal)) > date('Y-m-d',strtotime($tgl_akhir))){
				$this->session->set_flashdata('statuserror', 'Periode tanggal yang anda pilih tidak benar');
				redirect(base_url().'index.php/admin/lap_sparepart','refresh');
			}
			$data            = array();
			$sparepart       = $this->Md_pengeluaran->getPengeluaranTokoAndRangeDate($tgl_awal, $tgl_akhir);
			$pageData['excel'] = array(
				'tgl_awal'  => $tgl_awal,
				'tgl_akhir' => $tgl_akhir
			);
		...
		//select option view
		if ($argv1 == 'excel'){
			if (!$pageData['excel']) redirect(base_url().'index.php/admin/lap_sparepart','refresh');
			$this->load->view('page/excel_lap_sparepart', $pageData);
		}else{
			$pageData['page_name'] = 'lap_sparepart'; 
			$this->load->view('index', $pageData);
		}		
	}
